  ...
  class Connection < ActionCable::Connection::Base
    ...
    private
      def find_verified_user
        # Extract session from cookie (same as HTTP requests)
        if session = Session.find_by_token(cookies.signed[:session_token])
          # Extract account from the WebSocket URL path
          account = Account.find_by_external_id(request.path.split('/')[1])

          # Return the user for this account
          session.identity.users.find_by(account: account)
        end
      end
  end
end
