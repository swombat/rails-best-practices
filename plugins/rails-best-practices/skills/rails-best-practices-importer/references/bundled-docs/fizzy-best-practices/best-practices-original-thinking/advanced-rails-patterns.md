...
module MultiTenantJobExtensions
  ...
  def serialize
    super.merge("account" => @account&.to_global_id&.to_s)
  end
...
ActiveSupport.on_load(:active_job) do
  prepend MultiTenantJobExtensions
end
```
...
production: &production
  ...
  auto_archive_stale_cards:
    command: "Card.auto_archive_due"
...
```ruby
# app/jobs/concerns/smtp_error_handling.rb
module SmtpErrorHandling
  extend ActiveSupport::Concern
  ...
  included do
    ...
    rescue_from Net::SMTPSyntaxError do |error|
      ...
      when /\A501 5\.1\.3/  # Bad recipient address
        # Log but don't retry - address is permanently invalid
        Rails.logger.warn "Invalid email address: #{error.message}"
      else
    ...
    rescue_from Net::SMTPAuthenticationError do |error|
      # Alert immediately - this is a configuration problem
...
class Storage::Entry < ApplicationRecord
  ...
  def self.record(delta:, operation:, account:, board: nil, blob: nil)
    ...
    board&.materialize_storage_later
  end
end
```
...
```ruby
# app/mailers/concerns/unsubscribable.rb
...
```ruby
# app/controllers/concerns/timezone_aware.rb
module TimezoneAware
  extend ActiveSupport::Concern

  included do
    around_action :apply_timezone
    etag { timezone_from_cookie }  # Vary cache by timezone
  end

  private

  def apply_timezone(&block)
...
class User < ApplicationRecord
  ...
  def local_time(time)
    time.in_time_zone(timezone || "UTC")
  end
end
```
